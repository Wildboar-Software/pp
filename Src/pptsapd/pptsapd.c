...
# ifndef lint
static char Rcsid[] = "@(#)$Header: /xtel/pp/pp-beta/Src/pptsapd/RCS/pptsapd.c,v 6.0 1991/12/18 20:27:31 jpo Rel $";
# endif

/*
 * $Header: /xtel/pp/pp-beta/Src/pptsapd/RCS/pptsapd.c,v 6.0 1991/12/18 20:27:31 jpo Rel $
 *
 * $Log: pptsapd.c,v $
...
extern int  errno;
extern char	*pptsapd_addr, *pptailor;
/*  */
...
char **argv;
{
    ...
    while ((opt = getopt (argc, argv, "c:t:")) != EOF) {
		...
		case 't':
		    ...
		    break;
		default:
		    adios (NULLCP, "Usage %s [-t tailor] [-c address]");
		    break;
	    }
    }
    sys_init (myname);
    ...
    if (taddr == NULLCP)
	    taddr = pptsapd_addr;
    if ((tz = str2taddr (taddr)) == NULLTA)
	    adios (NULLCP, "Unknown transport address '%s'", taddr);
}
